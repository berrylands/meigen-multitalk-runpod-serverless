"""
RunPod Serverless Handler - V47 Debug Input Structure
"""
import os
import sys
import json
...
def handler(job):
    """RunPod handler function - receives job object"""
    try:
        logger.info("=== HANDLER CALLED ===")
        logger.info(f"Job type: {type(job)}")
        logger.info(f"Job contents: {job}")
        
        # Extract input based on RunPod structure
        if isinstance(job, dict):
            ...
            job_input = getattr(job, 'input', {})
        
        logger.info(f"Extracted job_input type: {type(job_input)}")
        ...
        # Process the request
        return process_request_safe(job_input)
        
    except Exception as e:
        logger.error(f"Handler error: {e}")
        logger.error(traceback.format_exc())
        return generate_fallback_response(f"Handler error: {str(e)}")
...
def process_request_safe(job_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Input keys: {list(job_input.keys()) if isinstance(job_input, dict) else 'Not a dict'}")
        logger.info(f"Full input: {json.dumps(job_input, indent=2) if isinstance(job_input, dict) else str(job_input)}")
        
        # Extract inputs with multiple fallbacks
        audio_filename = None
        image_filename = None
        
        # Try different parameter names
        audio_keys = ['audio_1', 'audio', 'audio_file', 'audio_path']
        image_keys = ['condition_image', 'reference_image', 'image', 'image_file', 'image_path']
        
        for key in audio_keys:
            if key in job_input and job_input[key]:
                audio_filename = job_input[key]
                logger.info(f"Found audio using key '{key}': {audio_filename}")
                break
        
        for key in image_keys:
            if key in job_input and job_input[key]:
                image_filename = job_input[key]
                logger.info(f"Found image using key '{key}': {image_filename}")
                break
        
        prompt = job_input.get('prompt', 'A person talking naturally')
        
        if not audio_filename:
            logger.error(f"No audio found. Checked keys: {audio_keys}")
            logger.error(f"Available keys in input: {list(job_input.keys())}")
            return generate_fallback_response(f"Missing audio input. Expected one of: {audio_keys}")
        
        if not image_filename:
            logger.error(f"No image found. Checked keys: {image_keys}")
            logger.error(f"Available keys in input: {list(job_input.keys())}")
            return generate_fallback_response(f"Missing image input. Expected one of: {image_keys}")
        
        logger.info(f"Processing: audio={audio_filename}, image={image_filename}, prompt={prompt}")
...
def main():
    ...
    try:
        logger.info("=== MULTITALK V47 DEBUG HANDLER STARTING ===")
        log_environment()
        ...
        if HAS_RUNPOD:
            ...
            logger.error("RunPod not available - running in test mode")
            # Test with sample input
            test_input = {
                "input": {
                    "audio_1": "1.wav",
                    "condition_image": "1.png",
                    "prompt": "Test prompt"
                }
            }
