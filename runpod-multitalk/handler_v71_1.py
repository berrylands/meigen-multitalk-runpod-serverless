...
# Import V71.1 pipeline
from multitalk_v71_1_official_wrapper import MultiTalkV71Pipeline, S3Handler
...
# Constants
VERSION = "71.1.0"
BUILD_ID = os.environ.get("BUILD_ID", "multitalk-v71.1-fixed")
...
def init_multitalk():
    ...
    if multitalk_pipeline is None:
        logger.info("Initializing MultiTalk V71.1 with model path: %s", MODEL_PATH)
        try:
            multitalk_pipeline = MultiTalkV71Pipeline(model_path=MODEL_PATH)
            logger.info("MultiTalk V71.1 pipeline initialized")
    ...
    try:
        ...
        if result["success"]:
            ...
            return {
                ...
                "message": f"Generated with MultiTalk V71.1 using official implementation (fixed input handling)"
            }
        else:
...
if __name__ == "__main__":
    ...
    logger.info("=== MULTITALK V71.1 HANDLER STARTING ===")
