"""
MultiTalk V69 - Improved Weight Mapping
Fixes the audio projection weight application to use all 10 weights instead of just 2
"""
import os
import sys
import torch
...
class MultiTalkV69Pipeline:
    """MultiTalk V69 - Improved Weight Mapping for Audio Projection"""
    ...
    def __init__(self, model_path: str = "/runpod-volume/models"):
        ...
        logger.info(f"Initializing MultiTalk V69 Improved Weight Mapping on {self.device}")
    ...
    def _initialize(self):
        ...
        # 1. Load VAE (working version from v68)
        ...
        # 4. Load and apply MultiTalk weights with improved mapping
        ...
        logger.info("✓ MultiTalk V69 initialized successfully")
        
    def _load_working_vae(self, model_path: Path):
        """Load the working VAE from v68"""
        try:
    ...
    def _load_and_apply_multitalk_weights(self, model_path: Path):
        """Load MultiTalk weights and apply to audio projector with improved mapping"""
        ...
        try:
            if multitalk_path.exists():
                ...
                # Create audio projector and apply weights with improved mapping
                self._create_and_apply_audio_projector_improved()
            else:
    ...
    def _create_and_apply_audio_projector_improved(self):
        """Create audio projector and apply MultiTalk weights with improved mapping logic"""
        try:
            self.audio_projector = AudioProjectionLayer().to(self.device).to(self.dtype)
            
            # Debug: Log all parameter names and shapes
            logger.info("=== AudioProjectionLayer Parameters ===")
            for name, param in self.audio_projector.named_parameters():
                logger.info(f"  {name}: {param.shape}")
            
            # Debug: Log relevant MultiTalk weight keys and shapes
            logger.info("=== MultiTalk audio_proj weights ===")
            audio_proj_weights = {k: v for k, v in self.multitalk_weights.items() if 'audio_proj' in k}
            for key, weight in audio_proj_weights.items():
                logger.info(f"  {key}: {weight.shape}")
            
            applied_count = 0
            
            # Create direct mapping table
            weight_mapping = {
                # LayerNorm weights
                'norm.weight': 'audio_proj.norm.weight',
                'norm.bias': 'audio_proj.norm.bias',
                
                # Linear layer weights
                'proj1.weight': 'audio_proj.proj1.weight',
                'proj1.bias': 'audio_proj.proj1.bias',
                'proj1_vf.weight': 'audio_proj.proj1_vf.weight',
                'proj1_vf.bias': 'audio_proj.proj1_vf.bias', 
                'proj2.weight': 'audio_proj.proj2.weight',
                'proj2.bias': 'audio_proj.proj2.bias',
                'proj3.weight': 'audio_proj.proj3.weight',
                'proj3.bias': 'audio_proj.proj3.bias',
            }
            
            # Apply weights using direct mapping
            for param_name, mt_key in weight_mapping.items():
                if mt_key in self.multitalk_weights:
                    mt_weight = self.multitalk_weights[mt_key]
                    
                    # Find the corresponding parameter
                    param = None
                    for name, p in self.audio_projector.named_parameters():
                        if name == param_name:
                            param = p
                            break
                    
                    if param is not None:
                        if param.shape == mt_weight.shape:
                            param.data = mt_weight.to(param.device).to(param.dtype)
                            applied_count += 1
                            logger.info(f"✓ Applied: {mt_key} -> {param_name} (shape: {mt_weight.shape})")
                        else:
                            logger.warning(f"Shape mismatch: {mt_key} {mt_weight.shape} vs {param_name} {param.shape}")
                    else:
                        logger.warning(f"Parameter not found: {param_name}")
                else:
                    logger.warning(f"MultiTalk weight not found: {mt_key}")
                    
            logger.info(f"Applied {applied_count}/10 audio projection weights to MultiTalk V69")
            
            if applied_count >= 8:  # Expect at least 8/10 weights (norm + proj layers)
                logger.info("✓ Successfully applied most audio projection weights")
            else:
                logger.warning(f"Only applied {applied_count}/10 expected weights - may affect quality")
    ...
    def process_audio_to_video(
        ...
        **kwargs
        ...
        """Process audio and image to generate video with MultiTalk V69"""
        try:
            logger.info("Processing with MultiTalk V69 Improved Weight Mapping...")
            ...
            for t in range(min(T, num_frames)):
                ...
                # Add debug info
                cv2.putText(frame, f"V69 Frame {t+1}", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(frame, "Improved Weights", (10, 60),
            ...
            return {
                ...
                "model": "multitalk-v69-improved-weights",
                "num_frames": len(frames),
                "fps": fps,
                "processing_time": processing_time,
                "architecture": "Memory-Efficient DiT + Improved Audio Projection Weights",
                "model_info": {
                    ...
                    "improved_weight_mapping": True
                }
            }
            
        except Exception as e:
            logger.error(f"Error in MultiTalk V69 processing: {e}")
            import traceback
