"""
MultiTalk V60 - Resolution/Attention Fix
Fixed spatial resolution mismatch in UNet attention layers
"""
import os
import sys
import torch
...
class MultiTalkConfig:
    ...
    num_frames: int = 81
    fps: int = 25
    resolution: int = 480  # V60 FIX: Changed from 512 to 480 to match WAN model
...
class WANDiffusionPipeline:
    ...
    def __init__(self, config: MultiTalkConfig):
        ...
        # Text projection layer for dimension matching
    ...
    def _load_vae(self, model_path: Path) -> bool:
        ...
        try:
            ...
            if vae_path.exists():
                ...
                # Initialize VAE with correct configuration for 480p
                self.vae = AutoencoderKL(
                    ...
                    layers_per_block=2,
                    latent_channels=8,
                    sample_size=self.config.resolution  # 480
                )
    ...
    def _load_unet(self, model_path: Path) -> bool:
        ...
        try:
            # V60 FIX: Try to load config.json to get proper UNet configuration
            ...
            if unet_config_path.exists():
                ...
                logger.info("✓ UNet loaded from diffusers config")
                return True
            ...
            if index_file.exists():
                ...
                # V60 FIX: Corrected UNet configuration for 480p
                # The error suggests we need different spatial dimensions
                self.unet = UNet2DConditionModel(
                    sample_size=60,  # 480 / 8 = 60 (was 64 for 512)
                    in_channels=8,
                    out_channels=8,
                    layers_per_block=2,
                    ...
                    # V60 FIX: Add these parameters for better compatibility
                ...
                logger.info(f"✓ UNet loaded from {len(loaded_files)} files (480p configuration)")
                return True
    ...
    def generate_frames(
        ...
        with torch.no_grad():
            ...
            # Project to UNet dimension (768-dim)
        ...
        for frame_idx in range(num_frames):
            ...
            for t in self.scheduler.timesteps:
                ...
                # V60 DEBUG: Log shapes
                if frame_idx == 0 and t == self.scheduler.timesteps[0]:
...
class MultiTalkV60Pipeline:
    """Complete MultiTalk V60 implementation - Resolution fixes"""
    
    def __init__(self, model_path: str = "/runpod-volume/models"):
        self.config = MultiTalkConfig(model_path=model_path)
        self.device = torch.device(self.config.device)
        
        logger.info(f"Initializing MultiTalk V60 on {self.device}")
        logger.info(f"Model path: {model_path}")
        logger.info(f"Resolution: {self.config.resolution}p")
    ...
    def process_audio_to_video(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"Reference image tensor shape: {image_tensor.shape}")
            ...
            logger.info(f"Reference latent shape: {reference_latent.shape}")
            ...
            return {
                ...
                "model": "multitalk-v60-resolution-fix",
                "num_frames": len(frames),
                "fps": fps,
                "architecture": "Full MultiTalk with WAN Diffusion (Resolution Fixed)"
            }
                
        except Exception as e:
            logger.error(f"Error in MultiTalk V60 processing: {e}")
            import traceback
            logger.error(traceback.format_exc())
