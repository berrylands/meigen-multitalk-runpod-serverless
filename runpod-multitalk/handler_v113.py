...
print("="*50)
print("V113: MultiTalk Handler - Complete MeiGen-MultiTalk Implementation")
...
try:
    ...
    if multitalk_v113.load_models():
        print("✓ MultiTalk V113 initialized successfully")
        
        # Show model info
        model_info = multitalk_v113.get_model_info()
        print(f"Models loaded: {sum(1 for v in model_info['models_loaded'].values() if v)}/5")
        ...
        print("✗ MultiTalk V113 initialization failed")
        multitalk_v113 = None
...
def download_from_s3(s3_url):
    ...
    try:
        ...
        if not all([aws_access_key, aws_secret_key]):
            print("V111: Missing AWS credentials for S3 download")
            return None
        ...
        print(f"V113: Downloaded {s3_url} to {temp_file.name}")
        return temp_file.name
        
    except Exception as e:
        print(f"V113: S3 download error: {e}")
        return None
...
def upload_to_s3(file_path, s3_key):
    ...
    try:
        ...
        if not all([aws_access_key, aws_secret_key, bucket_name]):
            print("V113: Missing S3 credentials")
            return None
        ...
        print(f"V113: S3 upload error: {e}")
        return None
    ...
    try:
        if isinstance(input_ref, str):
            ...
            # Simple filename - download from S3 bucket
                ...
                # Treat as S3 key in the configured bucket
                bucket_name = os.getenv('AWS_S3_BUCKET_NAME')
                if bucket_name:
                    s3_url = f"s3://{bucket_name}/{input_ref}"
                    return download_from_s3(s3_url)
                else:
                    print(f"V113: No S3 bucket configured for file: {input_ref}")
                    return None
        
        return None
        
    except Exception as e:
        print(f"V113: Error processing input: {e}")
        return None
...
def handler(job):
    """V113 Handler - Complete MeiGen-MultiTalk Implementation"""
        ...
        if action == "volume_explore":
            ...
            return {
                "output": {
                    ...
                    "message": "Network volume exploration completed (V113)"
                }
            }
            ...
            # Add MultiTalk V113 specific info
            if multitalk_v113:
            ...
            return {
                "output": {
                    "status": "ready",
                    "message": "V113 MultiTalk handler ready (Complete MeiGen-MultiTalk implementation)",
                    "version": "113",
            ...
            s3_key = job_input.get("s3_output_key", f"multitalk-v113/output-{int(time.time())}.mp4")
            ...
            # Check if MultiTalk V113 is available
            if not multitalk_v113:
                return {
                    "output": {
                        "status": "error",
                        "error": "MultiTalk V113 not available",
                        "details": {
                            "multitalk_v113_available": MULTITALK_V113_AVAILABLE,
                            "models_available": multitalk_v113.models_available if multitalk_v113 else None
                        }
                    }
                }
            ...
            print(f"V113: Processing audio: {audio_path}")
            print(f"V113: Processing image: {image_path}")
            print(f"V113: Prompt: {prompt}")
            
            # Generate video with MultiTalk V113
            ...
            if not video_path or not os.path.exists(video_path):
                return {
                    "output": {
                        "status": "error",
                        "error": "Failed to generate video with MultiTalk V113",
                        "details": {
            ...
            print(f"V113: Video generated: {video_path}, size: {os.path.getsize(video_path)} bytes")
            
            # Handle output format
            if output_format == "s3":
                s3_key = s3_key.replace("{version}", "v113")
                ...
                if video_url:
                    result = {
                        ...
                        "video_url": video_url,
                        "s3_key": s3_key,
                        "message": f"Video generated successfully with MultiTalk V113 (Complete MeiGen implementation)",
                        "generation_params": {
                            "prompt": prompt,
                            "sample_steps": sample_steps,
                            "text_guidance_scale": text_guidance_scale,
                    ...
                    result = {
                        ...
                        "message": "Video generated with MultiTalk V113 (S3 upload failed)",
                        "generation_params": {
                            "prompt": prompt,
                            "sample_steps": sample_steps,
                            "text_guidance_scale": text_guidance_scale,
...
# Start handler
print("V113: Starting RunPod serverless handler...")
