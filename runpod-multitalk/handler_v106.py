...
print("="*50)
print("V106: MultiTalk Handler Starting")
...
# Test all imports including xfuser
try:
    import xfuser
...
# Try importing MultiTalk components
try:
    from wan import MultiTalkPipeline
    print("✓ MultiTalk pipeline imported successfully")
    MULTITALK_AVAILABLE = True
except ImportError as e:
    print(f"✗ MultiTalk import failed: {e}")
...
def download_from_s3(s3_url):
    ...
    try:
        ...
        print(f"V106: S3 download error: {e}")
        return None
...
def upload_to_s3(file_path, s3_key):
    ...
    try:
        ...
        if not all([aws_access_key, aws_secret_key, bucket_name]):
            print("V106: Missing S3 credentials")
            return None
        ...
        print(f"V106: S3 upload error: {e}")
        return None
...
def process_input_file(input_ref, file_type):
    ...
    try:
        ...
        print(f"V106: Error processing input: {e}")
        return None
...
def create_test_video(audio_path, image_path):
    ...
    try:
        ...
        print("V106: Creating test video...")
        ...
        for i in range(75):
            frame = img.copy()
            cv2.putText(frame, f"MultiTalk V106 - {i+1}/75", (50, 50), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(frame, f"xfuser: {'YES' if XFUSER_AVAILABLE else 'NO'}", (50, 100),
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if XFUSER_AVAILABLE else (0, 0, 255), 2)
            out.write(frame)
        ...
        if audio_path and os.path.exists(audio_path):
            try:
                ...
                print(f"V106: Error adding audio: {e}")
                return temp_video
        
        return temp_video
        
    except Exception as e:
        print(f"V106: Error creating video: {e}")
        traceback.print_exc()
        return None

def handler(job):
    """V106 Handler - Fixed xfuser runtime dependencies"""
    print(f"V106: Received job: {job}")
    ...
    try:
        if action == "generate":
            ...
            print(f"V106: Processing audio: {audio_path}")
            print(f"V106: Processing image: {image_path}")
            
            # TODO: Use actual MultiTalk pipeline when models are available
            if MULTITALK_AVAILABLE and False:  # Disabled until models are loaded
                # pipeline = MultiTalkPipeline(...)
                # video_path = pipeline.generate(audio_path, image_path, prompt)
                pass
            else:
            ...
            print(f"V106: Video generated: {video_path}, size: {os.path.getsize(video_path)} bytes")
            
            # Handle output format
            if output_format == "s3":
                s3_key = s3_key.replace("{version}", "v106")
                ...
                if video_url:
                    result = {
                        ...
                        "video_url": video_url,
                        "s3_key": s3_key,
                        "message": f"Video generated successfully (V106 - xfuser {'available' if XFUSER_AVAILABLE else 'not available'})"
                    }
                else:
            ...
            model_info = {
                ...
                "multitalk_available": MULTITALK_AVAILABLE,
            ...
            return {
                "output": {
                    "status": "ready",
                    "message": "V106 MultiTalk handler ready (fixed xfuser dependencies)",
                    "version": "106",
...
# Start handler
print("V106: Starting RunPod serverless handler...")
