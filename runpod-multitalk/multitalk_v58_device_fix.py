"""
MultiTalk V58 - Device Mismatch Fix
Fixed device placement issues for proper GPU execution
"""
import os
import sys
import torch
...
class MultiTalkV58Pipeline:
    """Complete MultiTalk V58 implementation - Device fixes"""
    
    def __init__(self, model_path: str = "/runpod-volume/models"):
        self.config = MultiTalkConfig(model_path=model_path)
        self.device = torch.device(self.config.device)
        
        logger.info(f"Initializing MultiTalk V58 on {self.device}")
    ...
    def process_audio_to_video(
        ...
        **kwargs
        ...
        try:
            logger.info("Processing with MultiTalk V58...")
            ...
            return {
                ...
                "model": "multitalk-v58-device-fix",
                "num_frames": len(frames),
                "fps": fps,
                "architecture": "Full MultiTalk with WAN Diffusion (Device Fixed)"
            }
                
        except Exception as e:
            logger.error(f"Error in MultiTalk V58 processing: {e}")
            import traceback
            logger.error(traceback.format_exc())
