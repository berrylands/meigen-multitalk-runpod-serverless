...
print("="*50)
print("V110: MultiTalk Handler - Resilient Network Volume Explorer")
...
try:
    ...
    print("  Continuing without xfuser...")
...
def explore_network_volume():
    ...
    try:
        ...
        # Build directory tree (first 3 levels)
        def build_tree(path, max_depth=3, current_depth=0):
            ...
            try:
                ...
                tree[f"<error: {str(e)}>"] = {"type": "error"}
            
            return tree
        
        exploration["directory_tree"] = build_tree(model_path)
        
        # Look for WAN models
        wan_patterns = ["wan*", "WAN*", "*wan2.1*", "*i2v*", "*multitalk*", "*MultiTalk*"]
                    ...
                    try:
                        for item in wan_path.iterdir():
                            ...
                            if item.is_file():
                                ...
                                wan_info["size_mb"] += item.stat().st_size / (1024 * 1024)
                    except Exception as e:
                        wan_info["error"] = str(e)
                    exploration["wan_models"].append(wan_info)
        
        # Look for Wav2Vec models
        wav_patterns = ["wav2vec*", "Wav2Vec*", "*wav2vec*", "*chinese*", "*audio*"]
                ...
                if wav_path.is_dir():
                    ...
                    try:
                        for item in wav_path.iterdir():
                            ...
                            if item.is_file():
                                total_size += item.stat().st_size
                                wav_info["size_mb"] += item.stat().st_size / (1024 * 1024)
                    except Exception as e:
        ...
        for pattern in audio_patterns:
            for audio_path in model_path.rglob(pattern):
                if audio_path.is_file():
                    audio_info = {
                        "path": str(audio_path.relative_to(model_path)),
                        "size_kb": round(audio_path.stat().st_size / 1024, 2),
                        "parent_dir": audio_path.parent.name
                    }
                    exploration["sample_files"]["audio"].append(audio_info)
                    if len(exploration["sample_files"]["audio"]) >= 10:
                        break
        
        image_patterns = ["*.png", "*.jpg", "*.jpeg", "*.bmp"]
        for pattern in image_patterns:
            for image_path in model_path.rglob(pattern):
                if image_path.is_file():
                    image_info = {
                        "path": str(image_path.relative_to(model_path)),
                        "size_kb": round(image_path.stat().st_size / 1024, 2),
                        "parent_dir": image_path.parent.name
                    }
                    exploration["sample_files"]["images"].append(image_info)
                    if len(exploration["sample_files"]["images"]) >= 10:
                        break
        ...
        for item in model_path.iterdir():
            if item.is_dir():
                try:
                    ...
                    exploration["model_directories"][item.name] = {"error": str(e)}
        
        exploration["total_size_gb"] = round(total_size / (1024 ** 3), 2)
        
        # Sort checkpoints by size
        exploration["checkpoints"] = sorted(exploration["checkpoints"], key=lambda x: x["size_mb"], reverse=True)
...
# Print key findings
print(f"Volume exists: {volume_info.get('volume_exists', False)}")
print(f"Models directory exists: {volume_info.get('models_dir_exists', False)}")
print(f"Total size: {volume_info.get('total_size_gb', 0)} GB")
print(f"WAN models found: {len(volume_info.get('wan_models', []))}")
print(f"Checkpoints found: {len(volume_info.get('checkpoints', []))}")
print(f"Sample audio files: {len(volume_info.get('sample_files', {}).get('audio', []))}")
print(f"Sample image files: {len(volume_info.get('sample_files', {}).get('images', []))}")

# Show largest checkpoints
if volume_info.get('checkpoints'):
    print("\nLargest checkpoints:")
    for ckpt in volume_info['checkpoints'][:5]:
        print(f"  - {ckpt['path']} ({ckpt['size_mb']} MB)")

# Show WAN models
if volume_info.get('wan_models'):
    print("\nWAN models found:")
    for wan in volume_info['wan_models']:
        print(f"  - {wan['path']} ({wan.get('size_mb', 0):.1f} MB)")

print("="*50 + "\n")

# Import our MultiTalk implementation (with fallback)
...
try:
    ...
    print("  Continuing without MultiTalk inference...")
...
def download_from_s3(s3_url):
    ...
    try:
        ...
        print(f"V110: S3 download error: {e}")
        return None
...
def upload_to_s3(file_path, s3_key):
    ...
    try:
        ...
        if not all([aws_access_key, aws_secret_key, bucket_name]):
            print("V110: Missing S3 credentials")
            return None
        ...
        print(f"V110: S3 upload error: {e}")
        return None
...
def process_input_file(input_ref, file_type):
    ...
    try:
        ...
        print(f"V110: Error processing input: {e}")
        return None
...
def create_simple_test_video(audio_path, image_path):
    ...
    try:
        ...
        for i in range(75):
            ...
            cv2.putText(frame_bgr, f"V110 Test - {i+1}/75", (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame_bgr, f"xfuser: {'YES' if XFUSER_AVAILABLE else 'NO'}", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0) if XFUSER_AVAILABLE else (0, 0, 255), 2)
            out.write(frame_bgr)
        ...
        print(f"V110: Error creating test video: {e}")
        return None

def handler(job):
    """V110 Handler - Resilient Network Volume Explorer"""
    print(f"V110: Received job: {job}")
        ...
        if action == "volume_explore":
            ...
            return {
                "output": {
                    ...
                    "message": "Network volume exploration completed (V110)"
                }
            }
            ...
            print(f"V110: Processing audio: {audio_path}")
            print(f"V110: Processing image: {image_path}")
            
            # Generate video (fallback to simple test for now)
            ...
            print(f"V110: Video generated: {video_path}, size: {os.path.getsize(video_path)} bytes")
            
            # Handle output format
            if output_format == "s3":
                s3_key = s3_key.replace("{version}", "v110")
                ...
                if video_url:
                    result = {
                        ...
                        "video_url": video_url,
                        "s3_key": s3_key,
                        "message": f"Video generated successfully (V110 - resilient build)"
                    }
                else:
            ...
            if XFUSER_AVAILABLE:
                try:
                    ...
                    model_info["xfuser_version"] = "import failed"
            else:
            ...
            if XFORMERS_AVAILABLE:
                try:
                    ...
                    model_info["xformers_version"] = "import failed"
            else:
            ...
            return {
                "output": {
                    "status": "ready",
                    "message": "V110 MultiTalk handler ready (resilient build with volume explorer)",
                    "version": "110",
...
# Start handler
print("V110: Starting RunPod serverless handler...")
